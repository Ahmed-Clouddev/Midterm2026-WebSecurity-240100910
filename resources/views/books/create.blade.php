    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{ route('manage.books.store') }}" class="space-y-4">
                    @csrf
                    ...
                    <div>
                        ...
                        <x-text-input id="copies" name="copies" type="number" min="0" class="mt-1 block w-full" :value="old('copies', 0)" required />
                        <x-input-error :messages="$errors->get('copies')" class="mt-2" />
                    </div>

                    <x-primary-button>Save Book</x-primary-button>
                </form>
            </div>
        </div>
    </div>
